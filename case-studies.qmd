...
A key motivation for the stochastic formulation is that time-varying stochastic components in the observation process can absorb noise generated by model mis-specification, potentially improving robustness when the underlying mechanistic model is an approximation.
...
This composition yields the full observation model, which EpiAware uses to generate observations from infections:

```{julia}
#| echo: false
#| class-output: julia
I_t_demo = [0.01 for _ in 1:14]
print(@code_string generate_observations(transform_ascertainment, missing, I_t_demo))
```
...
fig_sir = let
    ...
    # Plot individual posterior trajectories (limit to n_samples to match Panel A)
    n_post_samples = min(n_samples, length(det_posterior_sols))

    # Deterministic model trajectories
    for sol in det_posterior_sols[1:n_post_samples]
        if sol.retcode == ReturnCode.Success
            lines!(ax21_S, sol.t, sol[1, :], color=(:blue, 0.1))  # S compartment
            lines!(ax21_I, sol.t, sol[2, :], color=(:blue, 0.1))  # I compartment
        end
    end

    # Stochastic model trajectories
    for sol in stoch_posterior_sols[1:n_post_samples]
        if sol.retcode == ReturnCode.Success
            lines!(ax21_S, sol.t, sol[1, :], color=(:orange, 0.1))  # S compartment
            lines!(ax21_I, sol.t, sol[2, :], color=(:orange, 0.1))  # I compartment
        end
    end
