...
Following *Chatzilena et al.*, we use log-normal and gamma priors for $\beta$ and $\gamma$, but with a tighter prior for the transmission rate (`LogNormal(0.25, 0.25)`) to reduce prior mass on extreme values.
...
sir_params = SIRParams(
    ...
    infectiousness = LogNormal(0.25, 0.25),
