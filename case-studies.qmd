...
fig_sir = let
    ...
    lines!(ax21_I, ts, det_I_q_vals[:, 3], color=:blue, linewidth=2, label="Det")
    band!(ax21_I, ts, det_I_q_vals[:, 2], det_I_q_vals[:, 4], color=(:blue, 0.3))

    lines!(ax21_I, ts, stoch_I_q_vals[:, 3], color=:orange, linewidth=2, label="Stoch")
    ...
    # Calculate Rt = β*S(t) for both models
    det_Rt = mapreduce(hcat, eachindex(det_posterior_sols)) do i
        sol = det_posterior_sols[i]
        if sol.retcode == ReturnCode.Success
            β = deterministic_sir_results.samples[:β][i]
            S_t = sol[1, :]
            β .* S_t
        else
            fill(NaN, length(ts))
    ...
    stoch_Rt = mapreduce(hcat, eachindex(stoch_posterior_sols)) do i
        sol = stoch_posterior_sols[i]
        if sol.retcode == ReturnCode.Success
            β = stochastic_sir_results.samples[:β][i]
            S_t = sol[1, :]
            β .* S_t
        else
            fill(NaN, length(ts))
