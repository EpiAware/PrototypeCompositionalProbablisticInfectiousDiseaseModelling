...
Y_t &\sim \text{Poisson}(\text{softplus}(I(t))).
\end{aligned}
$$
...
The softplus transformation ensures numerical stability when the solver returns small negative values.

*Chatzilena et al.* also present a stochastic model with time-varying ascertainment via an AR(1) process:

$$
\begin{aligned}
Y_t &\sim \text{Poisson}(\text{softplus}(I(t)) \exp(\kappa_t)), \\
...
We compose both models using `EpiProblem`, swapping the observation model component as in previous case studies.
We reuse the same `inference_method` defined in @sec-example1.
