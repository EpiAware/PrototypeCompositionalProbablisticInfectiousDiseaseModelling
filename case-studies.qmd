...
function solve_sir(β, γ, I₀, ts; tspan=(0.0, ts[end]))
    u0 = [1.0 - I₀, I₀, 0.0]
    p = [β, γ]
    prob = ODEProblem(EpiAware._sir_vf, u0, tspan, p)
...
fig_sir = let
    ...
    # Panel A: Prior SIR trajectories (sample from priors directly)
    sir_trajectory_plot!(ax11, sir_params, ts; n_samples=n_samples, alpha=0.1)
    ...
    for comp in [1, 2, 3]
        q_vals, c = compartment_quantiles(det_posterior_sols, ts, qs; compartment=comp)
        lines!(ax21, ts, q_vals[:, 3], color=c, linewidth=2)
        band!(ax21, ts, q_vals[:, 2], q_vals[:, 4], color=(c, 0.4))
        band!(ax21, ts, q_vals[:, 1], q_vals[:, 5], color=(c, 0.2))
    end

    # Panel E: Deterministic posterior predictive cases
    post_predictive_yt!(ax22, england_data, det_posterior_gens; qs)

    # Panel F: Stochastic posterior predictive cases
    post_predictive_yt!(ax23, england_data, stoch_posterior_gens; qs)
