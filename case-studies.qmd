...
Following *Chatzilena et al.*, we use log-normal and gamma priors for $\beta$ and $\gamma$, with a tighter prior for $\beta$ (`LogNormal(0.5, 0.25)`) to reduce prior mass on extreme values.
...
We use $\text{Gamma}(2, 1)$ which has a prior mean of 2 matching *Chatzilena et al.*, while ensuring the prior is bounded away from zero.
