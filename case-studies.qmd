...
Following *Chatzilena et al.*, we use log-normal and gamma priors for $\beta$ and $\gamma$.
For the recovery rate, *Chatzilena et al.* use $\text{Gamma}(0.004, 0.002)$ (shape, rate parameterisation), which has shape < 1 and places most prior mass near zero.
While this acts as a weakly informative prior, it creates numerical difficulties during sampling as $\gamma \to 0$ implies $R_0 \to \infty$.
We use $\text{Gamma}(2, 0.125)$ which has a prior mean of 0.25, implying a 4-day infectious period appropriate for influenza, while ensuring the prior is bounded away from zero.
For the transmission rate, we use $\text{LogNormal}(-0.5, 0.5)$ which, combined with our recovery rate prior, implies an $R_0$ prior with median approximately 3 and 10% prior probability of $R_0 < 1$.
