...
*Chatzilena et al.* present two observation models.
The deterministic model links the infected proportion directly to observed cases via a Poisson distribution:

$$
\begin{aligned}
Y_t &\sim \text{Poisson}(\lambda_t), \\
\lambda_t &= \text{softplus}(N \cdot I(t)/N).
\end{aligned}
$$

The stochastic model adds time-varying ascertainment via an AR(1) process:

$$
\begin{aligned}
Y_t &\sim \text{Poisson}(\lambda_t \exp(\kappa_t)), \\
\kappa_t &= \rho \kappa_{t-1} + \epsilon_t, \quad \epsilon_t \sim N(0, \sigma^2).
\end{aligned}
$$

This introduces `Ascertainment`, a new observation model modifier that applies a latent process as a multiplicative adjustment to expected observations.
We demonstrate both models below.
...
The ODE model returns the infected proportion $I(t)/N$, whilst the observation model handles the transformation to expected case counts.
...
For the stochastic model, we add time-varying ascertainment using `Ascertainment` with an AR(1) process.
We use `@set` from `Accessors.jl` to swap the underlying observation model:
...
transform_varying = @set transform_pois.model = varying_ascertainment
```
...
We swap the observation model using `@set`, demonstrating how `Ascertainment` composes with the same SIR process:

```{julia}
#| output: false
stochastic_sir = @set deterministic_sir.observation_model = transform_varying
```
