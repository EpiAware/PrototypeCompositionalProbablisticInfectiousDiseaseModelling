...
Following *Chatzilena et al.*, we use log-normal and gamma priors for $\beta$ and $\gamma$, but with a tighter scale parameter for the transmission rate (`LogNormal(0, 0.5)`) to reduce prior mass on extreme values.
We also use a more informative prior `Beta(1, 99)` for the initial proportion infected to concentrate prior mass near small outbreak initiations.
...
sir_params = SIRParams(
    ...
    infectiousness = LogNormal(0.0, 0.5),
...
fig_sir = let
    ...
    prior_ts = 1:(7*10)
    latent_infections = [500 * (1 + cospi(2 * t / 30.0)) for t in prior_ts]
