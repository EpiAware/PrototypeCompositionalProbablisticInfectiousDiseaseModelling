...
For the stochastic model, we add time-varying ascertainment using `Ascertainment`, which applies the log-scale AR(1) process $\kappa_t$ as a multiplicative adjustment via $\exp(\kappa_t)$.
We specify the AR process with weakly informative priors: damping near zero (strongly autocorrelated), initial state near zero (minimal baseline adjustment), and small innovation standard deviation.
