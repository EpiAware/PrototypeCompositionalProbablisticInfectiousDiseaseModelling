...
We use a $\text{Gamma}(6.5, 0.62)$ generation time distribution (matching the serial interval from @sec-example1 for comparison purposes, though in practice these would differ).
...
This code demonstrates component reuse (base `negbin` from @sec-example1), layered composition (adding day-of-week effects via `ascertainment_dayofweek`), and sequential composition (adding reporting delays via `LatentDelay`).
...
We create training data by subsetting the Italy data to match `tspan`.
We need to account for the extended time window required to handle reporting delays:

```{julia}
italy_training_data = (
    y_t = italy_data.y_t[(epinow2.tspan[1] - cutout):epinow2.tspan[2]],
    dates = italy_data.dates[(epinow2.tspan[1] - cutout):epinow2.tspan[2]],
    )
```
