...
Our replication composes an SIR compartmental model with time-varying transmission, an AR(1) latent process for stochastic deviations from deterministic dynamics, and a Poisson observation model for case counts.
In code, `ODEProcess` from the SciML ecosystem [@SciML] solves the SIR differential equations, `AR` generates multiplicative perturbations to the transmission rate, and `PoissonError` links the infected compartment to observed cases.
