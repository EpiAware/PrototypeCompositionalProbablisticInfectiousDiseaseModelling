...
Methods either chain together separate models, losing statistical information and introducing bias, or create monolithic models that require modellers to understand every component before making changes.
...
Current practice can achieve the required rigour through careful model development but may struggle to also have the flexibility needed to enable cross-domain collaboration and provide evidence in a timely manner.

Probabilistic programming languages (PPLs) such as Stan or LibBi [@Stan;@murray13_bayesian], and modelling frameworks such as pomp and monty [@king16_statistical;@fitzjohn25_monty], have enabled rigorous joint modelling and inference, reduced development effort and improved model quality compared with manual specification [@vandemeent2018introduction].
...
This can be seen in the fragmentation of the field.
...
To work around these flexibility constraints, some analyses choose to chain multiple models together, fitting separate models sequentially and passing outputs to subsequent stages [@huisman2022;@epinow2].
