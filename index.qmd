...
date: today
abstract: |
  Recent outbreaks of Ebola, COVID-19 and mpox have demonstrated the value of modelling for synthesising data for rapid evidence to inform decision making. Methods used to synthesise available data in real time broadly fall into two classes: either combining results from multiple, smaller models calibrated in isolation (the pipeline approach), or representing a single model tuned to the specific scenario (the joint model approach). Both approaches have significant downsides. Pipeline approaches combine results from separate models without uncertainty propagation, leading to loss of detail and statistical rigour. Joint approaches often use single monolithic models that, whilst powerful in their ability to systematically integrate multiple processes and data streams, are difficult to transfer to other settings or extend with additional model components. These barriers have prevented advances in settings where models could have provided actionable insights such as early outbreak analysis, wastewater surveillance, phylodynamics, clinical biomarkers, pooled testing, and large-scale clinical datasets. This paper presents a prototype framework that combines the modularity of pipeline approaches with the statistical rigour of joint models through composable epidemiological components. Our approach enables "LEGO-like" model building ands supports composability for multiple model types including discrete time models and ordinary differential equations. It can be readily extended for different computational backends. Through three case studies using the prototype, we show how components can be reused across different models whilst maintaining statistical rigour. The composable framework presented here enables adaptable modelling capabilities that can rapidly incorporate diverse data sources and domain expertise essential for future public health decision making.
format:
  pdf:
...
Recent developments in computational statistics and scientific computing demonstrate the potential for composable approaches that could address these limitations.
...
This provides dual interfaces: a high-level epidemiological interface for domain experts and a low-level Turing interface.
...
- Abstract types for generic model objects representing different epidemiological processes
- Clear interfaces between components maintaining statistical rigour
- Data-agnostic model definitions for enhanced generalisability
- Proper uncertainty propagation through standardised interfaces
- Ability to capture probabilities and uncertainty requires interfacing with probabilistic programming languages
- Stratification handling for arbitrary groupings (age, location, risk groups)
- Easy-to-use interfaces for mappings between process and observation models
- Support beyond ODEs: mixed equation types, abstract computational backends
- Integration with diverse approaches (agent-based models, symbolic-numeric computation)
- Interface enforcement through Julia's type system and Interfaces.jl
- Type stability for computational performance

## Design Philosophy {#sec-design}

- Epidemiological models as collections of interchangeable components
- Full uncertainty propagation through standardised interfaces
- Julia's type system and multiple dispatch for domain-specific language
- Complex models emerging from composing simpler, reusable components
- Dispatch at Turing level with support for arbitrary computational backends
- Architecture supports ODE solvers, agent-based models, symbolic-numeric systems
- Turing.jl chosen for: flexible probabilistic programming, robust submodel support
- Extensive inference algorithms, complex hierarchical structures, minimal boilerplate
- Eliminates "two-language problem": high-level specs compile to efficient code
- Julia's metaprogramming enables domain-specific abstractions hiding complexity
...
- Struct-in-struct composition pattern enables recursive model building
...
- Dual interfaces for different user needs

<!-- Reference to Panel B of Figure 1 -->
- ARIMA model demonstration as stack of reusable components
- ARIMA(p,d,q) constructed by composing: moving average (`MA`), autoregressive (`AR`), differencing (`DiffLatentModel`) components
...
```julia
@show DiffLatentModel
```

- ARIMA(2,1,1) composition by nesting components within each other
- "Struct-in-struct" pattern: complex models from simple component combinations
- MA model serves as error term for AR model, which becomes underlying process for differencing
...
arima_model = DiffLatentModel(
    ...
    model = AR(
        ...
        ϵ_t = MA(
            ma_priors = [truncated(Normal(0.0, 0.05), -1, 1)],
            init_priors = [Normal(0.0, 0.1)],
...
- Compositional design translates to concrete Turing models via `generate_latent` function
...
```julia
# Show the actual EpiAware implementation method
@show @which EpiAwareBase.generate_latent(AR(), 50)
```

- AR dynamics implemented through custom accumulation step maintaining autoregressive state

```julia
# Show the actual ARStep implementation
@show ARStep
# Show the callable function for ARStep
@show @which ARStep()(state, ϵ)
```

- MA model is similar

```julia
# Show the actual MA generate_latent implementation
@show @which EpiAwareBase.generate_latent(MA(), 50)
```

- The differenced process is purely deterministic...

```julia
# Show the actual DiffLatentModel generate_latent implementation
@show @which EpiAwareBase.generate_latent(DiffLatentModel(), 50)
```

- Composed ARIMA structure creates executable Turing model

```julia
@show generate_latent(arima_model, 50)
```

- Compositional design seamlessly translates abstract specifications to concrete probabilistic programs via multiple dispatch
- Backend implementation maintains full Turing compatibility
...
Compositional model specification with subsections. Give a very high levle overview of the model in text.
