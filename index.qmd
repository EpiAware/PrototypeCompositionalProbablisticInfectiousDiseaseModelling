...
Our replication uses the AR(2) latent model and negative binomial observation model defined in @sec-dsl, composing them with a renewal infection process that uses a discretised generation time distribution.
The `EpiProblem` container assembles these three components into a complete generative model.
The inference method combines Pathfinder variational inference for initialisation with NUTS sampling.
...
The `LatentDelay` struct adds incubation period and reporting delay convolutions using double interval censoring.
