...
```{julia}
tspan = (45, 80)
epi_prob = EpiProblem(epi_model = renewal,
    latent_model = ar2,
...
```{julia}
dayofweek_logit_ascert = ascertainment_dayofweek(negbin;  # Reuse negbin from Example 1
...
```{julia}
diff_ar = DiffLatentModel(ar2, Normal(); d = 1) # Normal prior for initial value of process
```
...
epi_prob_with_delay = EpiProblem(epi_model = renewal,      # Reused from Example 1
...
This `EpiProblem` reuses all three core components from Example 1: - **`renewal`**: The renewal-based epidemiological model with generation time distribution - **`ar2`**: The AR(2) latent process model for time-varying transmission rates, used in `diff_ar` - **`negbin`**: The negative binomial link model to data from expected observations.
...
longer_south_korea_data = (
    y_t = south_korea_data.y_t[(epi_prob_with_delay.tspan[1]):epi_prob_with_delay.tspan[2]],
    dates = south_korea_data.dates[(epi_prob_with_delay.tspan[1]):epi_prob_with_delay.tspan[2]],
    )
```
