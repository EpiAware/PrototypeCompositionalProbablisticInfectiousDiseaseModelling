...
- Internally this is creating a `AR` struct.
```{julia}
print(@code_string generate_latent(ar_model, 50))
```

-   Moving average component for error terms
...
- Again the constructor is creating a `MA` struct.
```{julia}
using Revise, CodeTracking
print(@code_string generate_latent(ma_model, 50))
```

-   Combining AR with MA error term creates ARMA model. Here we redefine the `AR` process so that we have a `ARMA(2, 1)`
...
-   Differencing the ARMA model produces the full ARIMA(2, 1, 1) structure as a composition
...
The constructor for this is just creating a `DiffLatentModel`
```{julia}
print(@code_string generate_latent(arima_model, 50))
```

-   Alternatively, EpiAware provides an `arima()` constructor function that simplifies this specification. Whilst not implemented we could avoid redefining the AR process using a `update` function.
...
```{julia}
print(@code_string EpiAwareBase.generate_latent(AR(), 50))
```
...
```{julia}
print(@code_string EpiAwareBase.generate_latent(MA(), 50))
```

-   The differenced process is purely deterministic...

```{julia}
print(@code_string EpiAwareBase.generate_latent(DiffLatentModel(AR(), Normal(); d=1), 50))
```

-   Again this has a composable step function

```{julia}
print(@code_string EpiLatentModels.MAStep([1, 2, 1])(1, 1))
```

-   Composed ARIMA structure creates executable Turing model by dispatching through each layer of the ARIMA struct. This means the code at the top level is the same as for the `DiffLatentModel`.
