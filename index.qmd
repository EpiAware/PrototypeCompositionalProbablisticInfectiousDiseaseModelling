...
We selected this study to demonstrate the three-component compositional structure central to our prototype: a latent process model (AR(2) for log $R_t$), an infection generating process (renewal equation), and an observation model (negative binomial link).
The relative simplicity of this model makes it an ideal first case study for demonstrating component modularity, transparent prior specification, and independent prior predictive checking of each component before composition into a joint model.
...
The `ar` model object defines the full sampling distribution, both for hyperparameters and trajectory, for $Z_t$.
...
Prior predictive samples (Figure @fig-posterior-preds-CS1, top left panel) show that *a priori* the _Mishra et al_ priors assign a few percent chance of achieving very high $R_t$ values, i.e. $R_t \sim 10-1000$ is not excluded.

#### Infection Generating Process

The renewal equation is defined in daily time increments, but delay distributions are commonly estimated and reported as continuous.
Our prototype provides a convenience constructor that converts continuous distributions into the discrete pmf $g_t$ required for the renewal model.
...
We construct a Turing model using `generate_epiaware` (introduced in @sec-backend) and condition on a fixed cluster factor value to improve identifiability.
