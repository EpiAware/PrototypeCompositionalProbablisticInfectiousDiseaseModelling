...
We assemble these components and use the No U-Turn Sampler (NUTS) [@hoffman2014no] for inference.
...
We recreate the single strain influenza example using data from @jombart2020outbreaks, introducing the SIR model as an alternative infection generating process to the renewal model used in the previous case studies.
...
The `EpiMethod` approach for chaining inference (combining pre-sampling like Pathfinder [@zhang2022pathfinder] with sampling like the No U-Turn Sampler (NUTS) [@hoffman2014no]) shows promise but should be a `Turing.jl` general solution rather than epi-specific because many domains face similar challenges of difficult posterior geometries where adaptive warmup strategies could improve both efficiency and reliability of inference across the broader probabilistic programming ecosystem.
