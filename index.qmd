...
- Of available options, only Julia PPLs provide necessary abstraction capabilities
- Turing.jl: highest adoption, largest team, best model composition support
- Light interface layer leveraging ecosystem vs reimplementing everything
- No lock-in: users work at high-level or drop to Turing level
- Distributed ecosystem increases sustainability: multiple institutions maintain different components
- Python/R entry points for accessibility, but core composability requires Julia's abstractions
- "ODE experts work on ODEs, GP experts on GPs, domain experts on domain" principle
- Abstraction layer handles automated bookwork: stratification, interface requirements, parameter hierarchies
