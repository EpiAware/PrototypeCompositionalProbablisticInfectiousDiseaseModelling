...
Our prototype could draw more heavily on category theory [@fong2018seven], which provides formal mathematical foundations for composability through operadic structures.
These operadic structures represent the algebraic framework for composing wiring diagrams [@vagner2014algebras] and guarantee valid model composition whilst maintaining modularity.
This approach has practical benefits for an epidemiological modeller, such as local changes to a model subcomponenet creating automatic modifications to the full composed model by abstracting away low level implementation.
The algebraic approach matches how modellers conceptualise new modelling approaches, that is typically as a kind of wiring diagram, whilst also providing automation for implementation.
...
These provide multiple interchangeable representations of model components as open petri nets, with connection "ports" allowing composibility with other components via "junction" porting [@libkind2022algebraic].
The operadic framework also enables automated hierarchical decomposition strategies for complex systems, such as when implementing population stratification or multiple strain modelling, and rigorous type systems that prevent invalid model compositions at compile time.
...
Additionally, `AlgebraicPetri.jl` uses the direct association between petri nets and reaction networks [@baez2017compositional] to link the graphical/algebraic representation of the model to its computational semantics via `ModelingToolkit.jl` [@modelingtoolkit2021] and `Catalyst.jl` [@loman2023catalyst].
`ModelingToolkit.jl` and `Catalyst.jl`, from the SciML ecosystem [@SciML], define dynamical systems with a symbolic-numeric approach, achieving substantial performance improvements through automated parallelisation, sparsity detection, and equation simplification, and therefore are an ideal computational endpoint for the model-level abstractions from AlgebricJulia.
However, AlgebraicJulia tools are currently designed for representing the generators of dynamical systems and do not naturally extend to general probabilistic models, requiring external tools like `Turing.jl` for Bayesian inference and additional layers to connect symbolic specifications to observational data with measurement error.
The essential incompatibility is due to the symmetric compositional structures considered in AlgebraicJulia, which is reflected in that the ordering of reactions do not affect the overall dynamics.
In contrast, general probabilistic models require a directed acyclic graph (DAG) representation to handle conditional dependencies and ensure correct sampling ordering.
While AlgebraicJulia can interface with external probabilistic programming tools, this creates a separation between the compositional structure and statistical inference that limits the seamless integration we achieve through our direct `Turing.jl` foundation.
