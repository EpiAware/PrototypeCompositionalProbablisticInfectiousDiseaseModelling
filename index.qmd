...
```{julia}
using Revise, CodeTracking, EpiAware
print(@code_string AR())
```

```{julia}
print(@code_string MA())
```

```{julia}
print(@code_string DiffLatentModel())
```
...
```{julia}
using Distributions
arima_model = DiffLatentModel(AR(;
...
```{julia}
print(@code_string EpiAwareBase.generate_latent(DiffLatentModel(), 50))
```
