...
### Generative model for case study 1
...
The `epi` model object defines the sampling distribution for $I_t$ defined in the [generative model](#generative-model-for-case-study-1) section, conditional on the time-varying reproductive number process $R_t$.
...
The full infection generating process, that is the model defined in the [generative model section](#generative-model-for-case-study-1) without the link to case data, can be constructed by passing samples of $Z_t$ into the renewal model sampler.
Since sampling information only passes from the latent process for $Z_t = \log R_t$ into the renewal model, at a high level this can be abstracted as a Bayesian network @heckerman1998tutorial, but at the level of fully coherent probabilistic/data-generative models rather simple parameteric random variables.
