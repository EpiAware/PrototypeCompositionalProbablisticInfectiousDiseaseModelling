...
Probabilistic programming languages (PPLs) such as Stan or LibBi [@Stan;@murray13_bayesian], and modelling frameworks such as pomp and monty [@king16_statistical;@fitzjohn25_monty], have not solved this problem.
These tools have enabled principled joint modelling and inference, reduced development effort and improved model quality compared with manual specification [@vandemeent2018introduction].
However, they generally do not provide straightforward ways to share or reuse model components; the practical unit of reuse remains the whole model or its codebase, not the underlying epidemiological concepts.
The resulting fragmentation is evident across the field.
For examples, at least seven packages estimate effective reproduction numbers using renewal-based approaches in Stan (`epidemia` [@scott2021_epidemia], `EpiNow2` [@epinow2], `epinowcast` [@epinowcast], `epimap` [@epimap], `EpiLine` [@epiline], `EpiSewer` [@lison2025episewer], `wwinference` [@johnson2025wwinference]), yet none share components despite overlapping features and, in some cases, the same authors.
Wastewater surveillance highlights challenges when different domain expertise is required: `EpiSewer` and `wwinference` were both implemented by non-wastewater experts with different levels of focus on sampling details, yet share no components and it is not clear which modelling choices matter.
...
Yet these, and other similar efforts, remain monolithic, difficult for users to contribute to or adapt beyond supported functionality, and limited in scope.
To work around these constraints, some analyses have adopted multi-stage "pipeline" approaches, fitting separate models sequentially and passing outputs to subsequent stages [@huisman2022;@epinow2].
This allows flexibility but prevents full propagation of uncertainty and can introduce biased estimates compared with fully joint models [@lison2024].
...
Advances in `Turing.jl`, a probabilistic programming language built in Julia [@Julia-2017], have introduced submodel interfaces that enable composable probabilistic programming, offering a pathway for epidemiological model composition [@Nicholson2022-ua; @fjelde2025turin].
In other fields, domain-specific languages (programming languages tailored to specific application domains that use terminology and concepts familiar to domain experts rather than general programming constructs) built in Julia have enabled compositional modelling: `HydroModels.jl` [@jing2025hydromodels] for hydrological modelling with differentiable neural-enhanced components, and `SpeedyWeather.jl` [@speedyweather2024] for building-block style atmospheric modelling with modular component assembly.
...
This would make it easier to fully propagate uncertainty and avoid implicit assumptions that lead to bias in current approaches.
...
A strength of our proposed approach is its modular design, which enables faster model development and component reuse whilst allowing comparison of modelling assumptions without significant reimplementation.
...
Category theory [@fong2018seven] provides formal mathematical foundations for composability, enabling automatic modifications to models when changes are made to underlying components.
...
However, these tools are designed for representing dynamical systems and do not currently extend to general probabilistic models.
Our probabilistic programming approach also offers more direct support for statistical inference and uncertainty quantification across a broader range of modelling approaches, but lacks the formal compositional guarantees of category theory and automated optimisations of symbolic-numeric frameworks.
...
In comparison to our proposed compositional DSL approach, domain-specific infectious disease tools currently provide useful features but are designed for a limited number of use cases and do not cover the full range of epidemiological modelling approaches needed in practice [@epinow2;@epinowcast;@scott2021_epidemia;@epiline].
Probabilistic programming languages (PPLs) commonly used in infectious disease modelling do not meet our proposed requirements for composability.
