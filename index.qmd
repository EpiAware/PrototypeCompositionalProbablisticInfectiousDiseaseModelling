...
Building on these frameworks, several efforts have aimed for greater generality within specific modelling paradigms such as flexible statistical models for real-time estimation [@scott2021_epidemia;@epinow2].
Yet these efforts, too, have encountered clear limits of extensibility: extending them requires embedding new data types or processes within a single, expanding codebase, which becomes harder to maintain and adapt as complexity grows.
To work around these constraints, some analyses have adopted multi-stage or “pipeline” approaches, in which separate models are fitted sequentially and their outputs passed as inputs to subsequent stages [@huisman2022].
This allows different data sources or processes to be analysed independently but prevents full propagation of uncertainty and can introduce biased estimates compared with fully joint models [@lison2024].
...
We demonstrate how our prototype compositional modelling DSL can recreate and extend existing epidemiological models through three case studies.
Each case study shows how complex models are built by composing reusable components, with full implementation details provided in the Supplementary Information.
...
All code and data for reproducing the analyses are available at: [https://github.com/EpiAware/PrototypeCompositionalProbablisticInfectiousDiseaseModelling](https://github.com/EpiAware/PrototypeCompositionalProbablisticInfectiousDiseaseModelling) and [https://github.com/sbfnk/EpiAwareR](https://github.com/sbfnk/EpiAwareR).
...
@mishra2020covid derive the renewal equation from age-dependent branching processes to provide a principled framework for estimating time-varying reproduction numbers from case data.
This foundational approach enables Bayesian hierarchical inference on reported case time series by combining the renewal equation with a negative binomial observation model.

Our replication uses the AR(2) latent model and negative binomial observation model defined in @sec-dsl, composing them with a renewal infection process [@cori2013new] that uses a discretised serial interval distribution [@wallinga2007generation].
The `EpiProblem` container assembles these three components into a complete generative model.
The inference method combines Pathfinder variational inference [@zhang2022pathfinder] for initialisation with NUTS sampling.

@fig-figure3 shows model components and posterior analysis.
...
![Model components and posterior analysis for Case Study 1. (A) Prior samples from the AR(2) latent process for log $R_t$ over 50 days. (B) Prior samples from the renewal model conditional on a fixed $R_t$ trajectory. (C) Prior samples from the negative binomial observation model around a latent infection curve. (D) Comparison of the continuous serial interval distribution with its discretised pmf. (E) Posterior predictive distribution for daily cases with 50% and 95% credible intervals. (F) Posterior predictive distribution for time-varying $R_t$ on a log scale.](figures/fig-figure3.pdf){#fig-figure3 width=100%}

## EpiNow2: Estimate Real-Time Case Counts and Time-Varying Epidemiological Parameters {#sec-example2}

`EpiNow2` [@abbott-epinow2-wellcomeopenres] addresses real-time estimation of case counts and epidemiological parameters accounting for reporting delays, right-truncation, and day-of-week effects.
These observation process complexities are common in surveillance data and must be handled appropriately for accurate inference.

Our replication reuses the AR(2), ARIMA(2,1,1), and negative binomial components from the main text and @sec-example1, extending them with new delay and temporal effect components.
The `broadcast_weekly` modifier converts the ARIMA(2,1,1) process to piecewise constant weekly values.
The `LatentDelay` struct adds incubation period and reporting delay convolutions using double interval censoring [@charniga2024best].
The `ascertainment_dayofweek` helper wraps the observation model with day-of-week effects using a softmax transformation.
...
This layered composition demonstrates how complex observation models can be built from simpler components.

@fig-figure4 shows model components and posterior analysis.
...
![Model components and posterior analysis for Case Study 2. (A) Prior samples from the piecewise constant by week ARIMA(2,1,1) latent process. (B) Prior samples from the renewal model. (C) Prior samples from the composite observation model including delays and day-of-week effects. (D) Incubation period and reporting delay distributions. (E) Posterior predictive distribution for daily cases. (F) Posterior predictive distribution for time-varying $R_t$.](figures/fig-figure4.pdf){#fig-figure4 width=100%}

## Contemporary statistical inference for infectious disease models using Stan {#sec-example3}

@chatzilena2019contemporary demonstrate contemporary statistical inference for infectious disease models, showing how to fit mechanistic compartmental models to outbreak data with proper uncertainty quantification.
Their work provides a template for integrating ODE-based transmission models with probabilistic observation processes.

Our replication integrates the SciML ecosystem [@SciML] for solving ordinary differential equations with EpiAware's observation model interface.
The AR(1) latent model captures stochastic deviations from the deterministic SIR dynamics, whilst the Poisson observation model links the infected compartment to observed cases.
This case study demonstrates that compositional approaches extend beyond renewal models to compartmental disease transmission models, enabling the same component reuse patterns across different modelling paradigms.

*The figure for this case study is currently under development and will be added once the implementation follows the standard case study format.*
