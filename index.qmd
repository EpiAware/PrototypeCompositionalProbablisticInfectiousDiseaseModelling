...
`Gen.jl` [@cusumanotowner2019gen] focuses lower-level implementation of probabilistic programs compared to `Turing.jl`, with composition support through nested generative function calls and combinators of generative functions.
...
Python-based probabilistic programming languages such as NumPyro [@phan2019composable] and Oryx [@oryx2020] could enable similar composability to our prototype [@pyrenew2024] along with combinator approaches [@coix2024], however by having a Julia-based [@Julia-2017] prototype our structural syntax can be largely divorced from computational implementation making backend extensions easier to develop.
...
As well as imperative probabilistic programming languages such as `Turing.jl`, there are declarative graph-based approaches available such as `JuliaBUGS.jl` [@sun2024juliabugs] and `RxInfer.jl` [@bagaev2023rxinfer] where users specify directed graphical models by explicitly stating conditional dependencies between variables.
