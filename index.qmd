...
The operadic framework also enables automated model transformations, hierarchical decomposition strategies for complex systems, and rigorous type systems that prevent invalid model compositions at compile time.
However, these category theory approaches are fundamentally designed for deterministic dynamical systems and do not naturally extend to general probabilistic programming languages, which require directed acyclic graph (DAG) representations to handle conditional dependencies and ensure well-defined probability measures.
