...
Our replication composes an AR(2) latent process for log $R_t$, a renewal infection process [@cori2013new] using a discretised serial interval distribution [@wallinga2007generation], and a negative binomial observation model for case counts.
In code, `AR` generates the latent log $R_t$ trajectory, `Renewal` computes expected infections via the renewal equation $I_t = R_t \sum_{s} g_s I_{t-s}$, and `NegativeBinomialError` links expected infections to observed cases with overdispersion.
The `EpiProblem` container assembles these three components, and inference combines Pathfinder [@zhang2022pathfinder] initialisation with NUTS sampling.
...
Our replication reuses the ARIMA(2,1,1) and negative binomial components from the main text and @sec-example1, extending them with piecewise constant weekly $R_t$ values, incubation period and reporting delay convolutions [@charniga2024best], and day-of-week reporting effects.
...
In code, `broadcast_weekly` converts the ARIMA(2,1,1) process to piecewise constant weekly $R_t$ values, `LatentDelay` convolves latent infections with incubation period and reporting delay distributions, and `ascertainment_dayofweek` multiplies expected observations by day-of-week effects via a softmax transformation.
