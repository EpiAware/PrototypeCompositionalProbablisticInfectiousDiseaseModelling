...
ar2 = AR(;
    ...
    ϵ_t=HierarchicalNormal(std_prior=HalfNormal(0.025))
)
display(ar2)
```
...
Prior predictive samples (Figure @fig-figure2 A) show that *a priori* these priors assign a few percent chance of achieving very high $R_t$ values, i.e. $R_t \sim 10-1000$ is not excluded.
...
Figure @fig-figure2 D compares the discretized generation interval with the underlying continuous distribution.
...
The implied distribution of $I_t$ trajectories conditional on this $R_t$ trajectory can be sampled independently of other model components (Figure @fig-figure2 B).
...
Prior predictive samples (Figure @fig-figure2 C) show the dispersion around the mean implied by our choice of prior.
...
function plot_post_pred(inference_data, inference_results; epi_prob,
    ...
    mdl_unconditional = generate_epiaware(epi_prob, (y_t=fill(missing, length(y_t)),))
...
Figure @fig-figure2 shows that the compositional model defined using our prototype system recovers the main finding in *Mishra et al*; that the $R_t$ in South Korea peaked at a very high value ($R_t \sim 10$ at peak, Figure @fig-figure2 F) before rapidly dropping below 1 in early March 2020, with the model capturing both the epidemic trajectory and day-to-day variation in case counts (Figure @fig-figure2 E).
...
We recreate the core EpiNow2 functionality by **reusing model components from @sec-example1** and **composing them with new delay and temporal effect components**.
...
We use the example dataset included with the EpiNow2 R package, which contains daily confirmed COVID-19 cases from Italy during the first wave in 2020.
This dataset spans from 22 February to 30 June 2020.
...
Our model reuses the renewal infection model and ARIMA(2,1,1) latent process from earlier sections, making it piecewise constant by week, whilst building a complex observation model that accounts for reporting delays and day-of-week effects.
...
The observation model layers three effects: negative binomial observation noise (reused from @sec-example1), day-of-week temporal modifiers, and reporting delays via convolution.
...
The piecewise constant weekly version is a configuration option in EpiNow2 but not the default.
Similarly, our piecewise constant weekly model applies the ARIMA(2,1,1) model to the increments $R_{w} - R_{w-1}$ at the weekly scale, then broadcasts this to daily values.
Prior predictive samples (Figure @fig-figure3 A) show the behaviour of this piecewise constant by week process.
...
EpiNow2 uses a Gamma distribution with uncertain parameters for the generation time: shape ~ Normal(1.4, 0.48) and rate ~ Normal(0.38, 0.25).
...
```{julia}
renewal_gt = Renewal(epinow2_data)
display(renewal_gt)
```

To demonstrate the infection model independently, we supply a fixed $R_t$ trajectory that decreases from 3 to 0.5 over 50 days.
Figure @fig-figure3 B shows prior samples from the renewal process conditional on this $R_t$ trajectory.
...
We build the observation model through **composition** of modular components reusing the negative binomial link observation model (`negbin`) from @sec-example1, and then layer additional modelling components on top.
...
EpiNow2 models two sequential delays: an incubation period (infection to symptom onset) and a reporting delay (symptom onset to case reporting).
The incubation period uses a LogNormal distribution with uncertain parameters: meanlog ~ Normal(1.6, 0.064) and sdlog ~ Normal(0.42, 0.069), giving a mean of 5.41 days at the parameter means.
...
Figure @fig-figure3 C demonstrates how these delay and day-of-week effects work together to transform a latent infection signal.
...
Unlike @sec-example1, we let the cluster factor parameter $\phi$ of the negative binomial distribution be jointly inferred rather than conditioning on a fixed value, following EpiNow2's approach.

```{julia}
epinow2_mdl = generate_epiaware(epinow2, italy_training_data)
```

We reuse the same `inference_method` defined in @sec-example1, showcasing how our inference setup can be applied across different model compositions.
...
Figure @fig-figure3 shows that the compositional model recovers similar $R_t$ dynamics to @sec-example1 (Figure @fig-figure3 F), whilst explicitly accounting for reporting delays (Figure @fig-figure3 D) and day-of-week effects that help disentangle true transmission changes from reporting artifacts (Figure @fig-figure3 E).
...
We recreate the single strain influenza examples from Chatzilena _et al_ by introducing the SIR model as an alternative infection generating process to the renewal model used in the previous case studies and composing this with temporal effects on ascertainment using an autoregressive latent process.
...
function posterior_R0(results)
    vec(results.samples[:β] ./ results.samples[:γ])
end
