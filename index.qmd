...
The probabilistic model for the latent infections $I_t$ is

$$
\begin{aligned}
Z_t & = \log R_t, \\
\rho_1, \rho_2, Z_0, Z_{-1}, I_0,  \sigma &\sim \pi(), \\
...
Where $\pi$ is a prior distribution for the hyperparmeters of the AR(2) model for the log-scale reproductive number and the initial conditions for the AR(2) and renewal trajectories.
...
We reuse the ARMA(2,1) model from above by reseting its noise model to independent normal with a hyperprior for $\sigma \sim \text{HalfNormal}(0.5)$.
This transforms the ARMA(2,1) model into the AR(2) model we desire.
...
We define the associated Turing model to `ar`, concretising its sampling period to 50 days for prior predictive modelling.
...
Using the decomposed models we can rapidly visualise that *a priori* the _Mishra et al_ example assigned a few percent chance of achieving very high $R_t$ values, i.e. $R_t \sim 10-1000$ is not excluded by our priors.
