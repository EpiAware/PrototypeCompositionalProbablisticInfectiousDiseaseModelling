...
First, we examine the autoregressive component structure, which models persistence through dependence on previous values.
The `AR` struct contains prior distributions for the autoregressive coefficients (`damp_priors`), initial conditions (`init_priors`), and the innovation process specification (`ϵ_t`).

```julia
@show AR
```

Next, the moving average component captures short-term dependencies through a weighted sum of recent innovations.
The `MA` struct requires the order specification, prior distributions for the moving average coefficients, initial values, and a hierarchical specification for the innovation variance.

```julia
@show MA
```

The differencing component transforms a non-stationary process into a stationary one by applying differences of order `d`.
The `DiffLatentModel` struct wraps any underlying latent model, enabling the composition of differenced processes.

```julia
@show DiffLatentModel
```

With these component definitions, we can now compose an ARIMA(2,1,1) model by nesting components within each other.
This demonstrates the "struct-in-struct" pattern where complex models emerge from simple component combinations, with the MA model serving as the error term for the AR model, which becomes the underlying process for differencing.
...
The struct-in-struct design enables component swapping for model comparison whilst maintaining mathematical consistency through the type system.
...
The compositional design translates to concrete Turing models through the `generate_latent` function that converts our abstract model specifications into executable probabilistic programs.
For the AR component, the implementation demonstrates how multiple dispatch enables seamless backend integration:

```julia
# Actual EpiAware implementation of generate_latent for AR models
...
The AR dynamics are implemented through a custom accumulation step that maintains the autoregressive state:

```julia
# The ARStep function that implements the AR dynamics
...
With our composed ARIMA structure defined, we can create an executable Turing model:

```julia
@show generate_latent(arima_model, 50)
```

This demonstrates how the compositional design seamlessly translates abstract model specifications into concrete probabilistic programs through multiple dispatch.

This backend implementation showcases how the abstract compositional design maintains full compatibility with Turing's probabilistic programming capabilities whilst enabling the modular "swap-in/swap-out" functionality that distinguishes our approach from monolithic alternatives.
