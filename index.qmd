...
Effective reproduction number estimation illustrates this: `epidemia` [@scott2021_epidemia], `EpiNow2` [@epinow2], `epinowcast` [@epinowcast], `epimap` [@epimap], `EpiLine` [@epiline], and `EpiSewer` [@lison2025episewer] all estimate Rt using renewal-based approaches in Stan, yet maintain separate codebases with no shared components despite overlapping features and, in some cases, the same authors.
Wastewater surveillance highlights challenges when different domain expertise is required: `EpiSewer` and `wwinference` [@johnson2025wwinference] both estimate transmission from wastewater in Stan but model different aspects, detailed sampling processes versus multiple sites, with no shared components.
