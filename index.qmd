...
Our replication reuses the ARIMA(2,1,1) and negative binomial components from the main text and @sec-example1.
We use `broadcast_weekly` to convert the ARIMA(2,1,1) process to piecewise constant weekly values, `LatentDelay` to add incubation period and reporting delay convolutions using double interval censoring [@charniga2024best], and `ascertainment_dayofweek` to wrap the observation model with day-of-week effects using a softmax transformation.
