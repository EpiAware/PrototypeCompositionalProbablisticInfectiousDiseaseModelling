...
To illustrate the compositional design patterns, we demonstrate how an ARIMA model can be built as a stack of reusable components.
In our framework, an ARIMA(p,d,q) process is constructed by composing a moving average (`MA`) component, an autoregressive (`AR`) component, and a differencing (`DiffLatentModel`) component through struct-in-struct nesting:

```julia
# Load EpiAware to access existing struct definitions
using EpiAware

# Display the actual struct definitions from the package
@show AR
@show DiffLatentModel

# Composition through nesting using existing EpiAware components
# ARIMA(1,1,1): MA(1) -> AR(1) -> Differencing(1)
arima_model = DiffLatentModel(
    d = 1,  # I(1) - first differencing component
    model = AR(
        damp_priors = [Normal(0.5, 0.2)],  # AR(1) parameters
        init_priors = [Normal(0.0, 1.0)],
        ϵ_t = MA(  # MA(1) component as innovation process
            ma_priors = [Normal(0.3, 0.1)],
...
# Show the composed structure
@show arima_model
```

This compositional structure enables component swapping for model comparison whilst maintaining mathematical consistency through the type system.

## Backend Implementation {#sec-backend}

<!-- Reference to Panel D of Figure 1 -->
Our DSL translates to concrete probabilistic models through multiple dispatch at the Turing.jl level, though the architecture supports abstraction to other computational backends.
Multiple dispatch enables flexible component combination where each struct type has corresponding methods that generate the appropriate Turing model behaviour.
The dispatch system automatically handles recursive composition, with each component contributing its probabilistic behaviour to the overall model whilst maintaining proper uncertainty propagation.

For the ARIMA example above, the framework generates corresponding Turing models through dispatch methods:

```julia
# Dispatch methods translate DSL to Turing models
function generate_latent(model::AR, n_timepoints)
    @model function ar_model()
        # Prior sampling for AR parameters
        damp_AR ~ product_distribution(model.damp_priors)
        ar_init ~ product_distribution(model.init_priors)
        ϵ_t ~ model.ϵ_t

        # AR process generation
        Z_t = Vector{Float64}(undef, n_timepoints)
        # ... implementation details
        return Z_t
    end
end

function generate_latent(model::DiffLatentModel, n_timepoints)
    # Recursively call inner model then apply differencing
    inner_model = generate_latent(model.model, n_timepoints + model.d)
    @model function diff_model()
        @submodel Z_inner = inner_model()
        Z_t = diff(Z_inner, model.d)
        return Z_t
    end
end
```

This backend abstraction design maintains the potential to target different computational frameworks such as ODE solvers, agent-based models, or symbolic-numeric systems whilst preserving the compositional interface structure.
