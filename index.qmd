...
We compose an SIR compartmental model using `ODEProcess` from the SciML ecosystem [@SciML] to solve the differential equations.
The ODE model returns infected proportions, whilst the observation model handles the transformation to expected case counts via `TransformObservationModel`.
We demonstrate two observation models: a deterministic model with direct Poisson link, and a stochastic model that introduces `Ascertainment`, an observation model modifier for time-varying effects using an AR(1) process.

@fig-sir shows model components and posterior analysis.
Panel A shows prior SIR trajectories.
Panel B demonstrates the stochastic observation model with time-varying ascertainment.
Panel C compares posterior $R_0$ distributions between deterministic and stochastic models.
Panel D shows posterior compartment trajectories.
Panels E-F compare posterior predictive cases between models, recovering similar findings to *Chatzilena et al.* with $R_0 \approx 2$.

![Model components and posterior analysis for Case Study 3. (A) Prior SIR trajectories showing S (blue), I (red), R (green) dynamics. (B) Stochastic observation model samples with time-varying ascertainment. (C) Posterior $R_0$ comparison between deterministic and stochastic models. (D) Posterior compartment trajectories. (E) Deterministic model posterior predictive cases. (F) Stochastic model posterior predictive cases.](figures/fig-sir){#fig-sir width=100%}
