...
*Panel B*: Posterior density plot with true values (i.e. pairs plot)
*Panel C*: Posterior predictive vs the sampled data.
...
```{julia}
summarystats(mishra_results.samples)[1:7, :]
```
...
First, the transformation and broadcasting generates:
...
And second on the action of ascertainment as a multiplier on the time series of latent infections along with the probabilistic model for $\tilde{\omega}$ and variable prefixing, which produces:
...
```{julia}
summarystats(epinow2_results.samples)[1:7, :]
```
