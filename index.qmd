...
`epidemia` and `epimap` saw development cease, `EpiLine` has minimal ongoing work, and whilst `EpiNow2` and `epinowcast` continue development, new features must be implemented separately in each.
...
We compose an SIR compartmental model with time-varying transmission, an AR(1) latent process for stochastic deviations from deterministic dynamics, and a Poisson observation model for case counts.
We use `ODEProcess` from the SciML ecosystem [@SciML] to solve the SIR differential equations, `AR` to generate multiplicative perturbations to the transmission rate, and `PoissonError` to link the infected compartment to observed cases.
