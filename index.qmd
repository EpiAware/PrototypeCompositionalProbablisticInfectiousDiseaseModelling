...
In this paper, we first identify requirements for composable infectious disease modelling based on the challenges outlined above, our experience developing infectious disease models during recent outbreaks, and established best practices for Bayesian modelling.
We then present a prototype that addresses these requirements, combining the modularity of pipeline approaches with the statistical rigour of joint models through composable epidemiological components.
Our approach enables "LEGO-like" model construction through standardised interfaces similar to those used in `SpeedyWeather.jl` [@speedyweather2024].
The prototype supports composability of a range of model families including ordinary differential equations and mixed equation types, with the potential for different computational backends, implemented as a domain-specific language intended to be implemented as optional package extensions [@juliapackageextensions].
...
Based on the challenges outlined above, our experience developing infectious disease models during recent outbreaks, and established best practices for Bayesian modelling [@gelman2020bayesian], we identify the following requirements for composable infectious disease modelling.
...
This approach enables "LEGO-like" model construction, maintaining the statistical rigour of joint models whilst providing the flexibility of chaining together simpler models.
