...
Based on the challenges outlined above, our experience developing infectious disease models during recent outbreaks, and established best practices for Bayesian modelling [@gelman2020bayesian], we propose the following requirements for composable infectious disease modelling.

Modelling infectious disease dynamics requires quantifying uncertainty at every level because decisions should account for incomplete knowledge about individual infection risk, transmission dynamics, and observation processes [@swallow2022chall].
...
Because diseases affect populations heterogeneously across age, location, and risk groups [@heesterbeek15_modeling], the framework needs to be able to support arbitrary stratification schemes for all components.
These stratified models should also remain data-agnostic as this allows the model to be generalised to different datasets, tested based on just its prior specification, and used for forecasting.
...
To allow for models to be validated, the framework should support nesting models within models and programming over model structure itself, allowing simple components to compose into sophisticated models while remaining individually interrogable for debugging, validation, and mechanistic understanding.
Treating models as a collection of submodels in this way also enables staged inference, enabling one model's outputs to serve as another's inputs with proper uncertainty propagation as formalised in Markov melding [@goudie2019markov] and cut likelihoods [@plummer2015cuts].
Model specifications should serve dual purposes, supporting both forward simulation for prior predictive checks and forecasting, and inference when conditioned on observed data, without requiring separate implementations [@gelman2020bayesian].
...
Supporting modern inference methods is important so that complex models can be fit.
This will generally necessitate gradient computation throughout via automatic differentiation.
...
This means supporting abstract back-ends to allow seamlessly switching between inference approaches.
...
Such interfaces would also enable large language models to serve as model construction agents, composing models from component libraries whilst validation methods ensure statistical rigour [@aygun2025ai].
...
Finally, we need a framework that can be composed with out of domain approaches and expertise, such as neural networks, Gaussian processes, and other machine learning approaches [@philipps2025ude].
