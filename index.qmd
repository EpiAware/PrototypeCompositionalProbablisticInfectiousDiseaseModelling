...
Similarly, our piecewise constant weekly model applies the ARMA(2,1) model to the increments $R_{w} - R_{w-1}$ at the weekly scale, then broadcasts this to daily values.
...
We reuse the `renewal` structure from @sec-example1, updating it to use the generation time distribution rather than the serial interval.
...
```{julia}
delay_padding = length(delay_dayofweek_negbin.rev_pmf) + length(incubation_dayofweek_negbin.rev_pmf)
...
We create training data by padding the start with missing observations for the delay period, using earlier dates:

```{julia}
padded_dates = vcat(
    [italy_data.dates[1] - Day(delay_padding - i + 1) for i in 1:delay_padding],
    italy_data.dates[1:50]
)

italy_training_data = (
    y_t = vcat(fill(missing, delay_padding), italy_data.y_t[1:50]),
    dates = padded_dates,
    )
```
...
figure3 = let
    ...
    barplot!(ax21, 0:(length(delay_pmf)-1), delay_pmf; label="Combined delay pmf")
    lines!(ax21, 0:0.1:20, x -> pdf(incubation_distribution, x);
        color=:green, linewidth=2, label="Incubation period")
    lines!(ax21, 0:0.1:20, x -> pdf(reporting_distribution, x);
        color=:blue, linewidth=2, label="Reporting delay")
    axislegend(ax21)
