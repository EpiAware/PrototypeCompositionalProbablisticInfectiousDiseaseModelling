...
<!-- Problem statement paragraph - inspired by grant applications -->
-   Recent outbreaks (Ebola, COVID-19, mpox) demonstrate need for rapid model development
-   Models must synthesise complex data for evidence-based decisions
-   Domain experts cannot contribute without understanding entire frameworks
-   Individual-level data (viral loads, biomarkers) ignored when informing population models
-   Barriers prevent integration of clinical, laboratory, and epidemiological expertise
-   Creates inefficient rebuilding rather than component reuse

<!-- Current approaches in epidemiology paragraph -->
-   Pipeline approaches: separate models lose uncertainty propagation
-   Joint models: monolithic, scenario-specific, hard to extend
-   Each outbreak requires rebuilding rather than reusing components
-   Model inflexibility identified by WHO, UKHSA, CDC as key barrier
-   Transfer of methodology between outbreaks proven difficult
-   Force choice between statistical rigour and flexibility

<!-- Solutions from other fields paragraph -->
-   Category theory: operadic composition for hierarchical model construction
-   SpeedyWeather.jl: "LEGO-like" atmospheric modelling with interactive DSL
-   ModelingToolkit.jl: abstract backends beyond ODEs (DAEs, PDEs, SDEs)
-   Turing.jl advances: submodel interfaces, composable probabilistic programming
-   Key insight: separation of syntax (composition) from semantics (computation)

<!-- This prototype work paragraph -->
-   Present prototype framework combining pipeline modularity with joint model rigour
-   Enable "swap-in/swap-out" model decisions through standardised interfaces
-   Support composability beyond ODEs: mixed equation types, abstract backends
-   Create DSL operating as package extension with dual interfaces:
    -   High-level DSL for epidemiologists (EpiProblem interface)
    -   Low-level Turing interface for methodologists
-   Demonstrate through three epidemiological case studies using EpiAware package
-   Show how domain experts can contribute components without full system knowledge
-   Bridge individual-level clinical data with population-level transmission models
