...
$g_t$ is the generation time distribution pmf and $\xi_t$ is the reporting delay distribution pmf (both obtained by double-censoring continuous distributions).
...
We reuse the `renewal` model from Example 1.
Figure @fig-posterior-preds-CS2 (top middle panel) shows prior samples from the renewal process with a fixed $R_t$ trajectory.
...
renewal_mdl = let
    ...
    epinow2_data_samples = (gen_int = epinow2_data.gen_int, )
    _renewal_mdl = generate_infections(renewal, rt_process, epinow2_data_samples)
end
```
...
This `EpiProblem` reuses the core epidemiological component from Example 1 (`renewal`) and the ARIMA(2,1,1) latent model from @sec-dsl (`arima211`).
