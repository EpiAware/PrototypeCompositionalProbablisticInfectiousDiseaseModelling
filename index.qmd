---
title: "The case for composable probabilistic infectious disease models"
authors:
  - name: "Sam Abbott"
    affiliations:
      - ref: lshtm
...
Methods either chain separate models, losing statistical information and introducing bias, or create monolithic models that require modellers to understand every component before making changes.
These barriers have prevented methodological advances across settings where models could provide actionable insights.

Composable models, where components can be reused across contexts and combined whilst maintaining statistical rigour, could address these challenges.
When components have standardised interfaces, domain experts can contribute specialised knowledge without reviewing entire modelling frameworks.
This enables the statistical rigour of joint models whilst maintaining the flexibility to adapt analyses as new data sources become available.

We propose requirements for composable infectious disease modelling based on challenges from recent outbreaks, including support for uncertainty quantification, separation of infection and observation processes, and programming over model structure to enable nesting and staged inference.
We demonstrate feasibility through a proof of concept that addresses these requirements, recreating three published analyses using shared components.
Such systems could transform outbreak response by enabling faster model adaptation, lowering barriers for interdisciplinary collaboration, and potentially enabling language model assisted model construction.
...
As a consequence, modellers often have to re-implement model components in new models that used for different situations even when underlying concepts are, in principle, the same.
...
These tools enable statistically principled joint modelling and inference with probabilistic reasoning across a wide range of applications.
...
Effective reproduction number estimation illustrates this: `epidemia` [@scott2021_epidemia], `EpiNow2` [@epinow2], `epinowcast` [@epinowcast], `epimap` [@epimap], `EpiLine` [@epiline], `EpiSewer` [@lison2025episewer], and `wwinference` [@johnson2025wwinference] all estimate Rt using renewal-based approaches in Stan, yet maintain separate codebases with no shared components despite overlapping features and, in some cases, the same authors.
Wastewater surveillance highlights challenges when different domain expertise is required: `EpiSewer` and `wwinference` both estimate transmission from wastewater in Stan but model different aspects, detailed sampling processes versus multiple sites, with no shared components.
...
Modelling making use of data from the UK Office for National Statistics Community Infection Survey (CIS) illustrates these challenges [@ons2020cis].
...
ONS estimated prevalence from their model applied to this data, but modellers without access to the underlying observations had to develop separate methods to estimate incidence from these published prevalence estimates [@abbott2022estimating].
These methods typically assumed that prevalence observations were independent with normally distributed uncertainty, and any subsequent uses inherited implicit assumptions such as assuming stationarity if a Gaussian process was used for estimation [@abbott2022estimating].
These incidence estimates were subsequently used as inputs to policy-relevant analyses of Delta variant emergence [@mccrone2022delta] and Omicron severity [@nyberg2022omicron].
Whilst other analyses that were used to inform policy fitted directly to the summarised prevalence estimates [@davies21_associationcovidengland].
Attempts were also made to estimate reproduction numbers directly from the estimated prevalence, but these suffered from oversmoothing and timing delays, likely due to not fully accounting for the complexities of the observation process [@mccabe2024inferring].
Here less ad-hoc joint modelling methods, of a similar standard to the high quality methods used in the CIS for data collection, could not be used due to the unavoidable data security constraints on the underlying data, siloing of highly specialised expertise, and the lack of consistent modelling language and tools across the different analyses.

Recent developments in computational statistics and scientific computing demonstrate the potential for composable modelling, in which components can be reused across contexts and combined in different configurations whilst maintaining statistical rigour, to address these challenges.
Advances in `Turing.jl` have introduced submodel interfaces that enable composable probabilistic programming, offering a pathway for epidemiological model composition [@Nicholson2022-ua; @fjelde2025turin].
However, these approaches have remaining challenges such as handling data at different temporal resolutions and spatial granularities, situations where moment-matched Gaussian approximations (used in their approximate Markov melding approach) prove inadequate, and determining how best to weight different evidence sources whilst incorporating prior knowledge.
Their work also focused on ad-hoc interoperable models for specific applications rather than a general system for question-agnostic interoperable modelling.
...
In other fields, these ideas have been applied to hydrology and atmospheric science: `HydroModels.jl` [@jing2025hydromodels] demonstrates compositional hydrological modelling with differentiable neural-enhanced components, whilst `SpeedyWeather.jl` uses an interactive domain-specific language (a programming language tailored to a specific application domain that uses terminology and concepts familiar to domain experts rather than general programming constructs) approach for building block style atmospheric modelling with modular component assembly [@speedyweather2024].
...
For the CIS example above, composability would allow the observation model linking prevalence surveys to underlying infection dynamics to be developed once and shared as a reusable submodel.
Approaches to combining such submodels, such as Markov melding [@goudie2019markov] and cut likelihoods [@plummer2015cuts], could then enable proper uncertainty propagation from initial surveillance estimates through to policy-relevant analyses, avoiding the implicit assumptions and cascading biases inherent in current approaches.

In this paper, we first propose requirements for composable infectious disease modelling based on the challenges outlined above, our experience developing infectious disease models during recent outbreaks, and established best practices for Bayesian modelling.
We then present a proof of concept that addresses these requirements, combining the modularity of pipeline approaches with the statistical rigour of joint models through composable epidemiological components.
...
Through three case studies using the prototype, we show how components can be reused across different models: the first replicates a COVID-19 analysis for South Korea using a renewal process with time-varying reproduction numbers [@mishra2020covid]; the second extends these components with reporting delays and day-of-week effects to replicate EpiNow2, a real-time nowcasting tool [@epinow2]; the third replicates an SIR model analysis of influenza outbreak data, integrating ODE solvers [@chatzilena2019contemporary].
...
The requirements in the uncertainty and inference theme and the model structure theme address the statistical foundations of building and fitting models with appropriate uncertainty quantification.
The requirements in the population heterogeneity theme address the need to model diseases across different groups whilst reducing the additional bookkeeping this typically requires.
The requirements in the accessibility and adoption theme lower barriers for diverse users, enable incremental uptake, and support the propagation of domain expertise into composed models.
The requirements in the interoperability theme ensure components work together and integrate with external tools.
The requirements in the modelling workflow theme support the development of validated models that can answer key stakeholder questions in decision maker relevant timelines [@whitty2015what], and enable emerging opportunities for large language model assisted model construction.
Whilst we have presented this as a mapping of problems to requirements, in reality there is overlap; for example, standardised interfaces enable component compatibility, support handling of multiple strata, and ensure proper uncertainty propagation.

::: {.landscape}

| Theme | Proposed requirement | Motivating problem |
|-------|---------------------|-------------------|
| **Uncertainty & inference** | Probabilistic model specification with full uncertainty quantification | Decisions must account for incomplete knowledge about infection risk, transmission, and observation [@swallow2022chall] |
...
| | Nesting models within models | Adding new data sources or processes can require modifying entire models |
| | Support for multiple modelling paradigms | Infectious disease models take many forms (ODEs, agent-based, network, stochastic, discrete time) |
...
| | Stratification bookkeeping abstracted from users | Manual bookkeeping for multiple strata can be complex and error-prone |
| | Programming over model structure | Replicating model components across strata often requires manual specification for each group |
| | Partial pooling [@gelman2007data] through relationships between strata | Sparse data in individual strata can require borrowing information between groups |
...
| | Incremental adoption paths | Requiring complete rebuilds of existing models can be wasteful and time-consuming [@whitty2015what] |
| | Components functional as standalone tools | Components should provide value even outside the compositional framework to maximise utility |
...
| | Composability with machine learning approaches | Integration with neural networks, Gaussian processes, and other ML tools enables leveraging that expertise [@philipps2025ude] |
| **Modelling workflow** | Dual-purpose specifications for simulation and inference | Both simulation (for prior predictive checks and forecasting) and inference are needed [@gelman2020bayesian], but implementing both separately can be resource intensive and difficult to keep in sync |
| | Rapid model iteration | Evidence is urgently needed during outbreaks and other time sensitive settings; iteration speed matters for decision making [@whitty2015what] |
| | Easy model and model component validation and exploration | Complex models are difficult to debug, validate, and understand |
...
: Proposed requirements for composable infectious disease modelling. {#tbl-requirements}
...
@fig-case-studies highlights component reuse across the case studies: Mishra et al. reuses the AR(2) component from the ARIMA example, the EpiNow2 replication reuses the full ARIMA structure, and Chatzilena et al. demonstrates an alternative SIR ODE infection process.
...
In this paper we have identified barriers in epidemiological modelling arising from the tension between statistical rigour and practical flexibility, proposed compositional infectious disease models as a solution, laid out a set of requirements for such frameworks, and showcased a proof of concept approach that meets these requirements.
...
Our three case studies, which were based on previous studies [@mishra2020covid; @epinow2; @chatzilena2019contemporary], demonstrate that this compositional framework can address real-world epidemiological problems across a range of modelling approaches and underlying methods.

<!-- Strengths and weaknesses interwoven -->

A strength of our proposed approach is its modular design, which enables faster model development and component reuse, whilst also allowing for comparison of modelling assumptions without significant reimplementation.
The approach enables researchers to embed new components within existing models more easily which reduces implementation barriers for methodological development.
...
Domain experts such as virologists, clinicians, or environmental scientists can contribute specialised components encoding their knowledge without needing to review other components or understand the programming details.
Whilst a limitation of our approach is that it requires immplementation in Julia, which is not currently common in the field, the `EpiAwareR` R interface [@EpiAwareR] demonstrates that our frameworks can bridge software ecosystems.
Similar interfaces could be built for other languages using existing Julia bridges such as `PythonCall.jl` [@PythonCall.jl] for Python.
Potentially, the ability to compose models will improve the quality and reliability of models constructed by large language models due to the same design features that make tinterdisciplinary collaboration easier.
However, a clear limitation is that our proposed approach is a proof of concept and not designed for ongoing use or widespread adoption by the field.
...
Similarly, we only added partial support for partial pooling approaches with implemented infrastructure in place to support users in using this functionality in our DSL layer.
The current component library also remains limited to relatively basic epidemiological patterns.
Finally, the current DSL layer manipulation tooling cannot coordinate updates of related parameters such as model order and corresponding priors, without manual intervention, requiring either avoiding this pattern or developing additional solutions.
Whilst our proof of concept has limitations, the three real-world case studies presented here establish the viability of our compositional approach for real-world epidemiological problems.
Another limitation is the learning curve required for researchers familiar with current approaches to adopt this compositional paradigm.
...
Similarly, ongoing work in the `Turing.jl` ecosystem should be able to identify and resolve performance bottlenecks that do emerge.
...
Users may not know whether problems originate in `Turing.jl`, our DSL layer, or elsewhere in the ecosystem, and even once located, it can be difficult to determine who is responsible for resolving them.
There are also additional technical barriers within the `Turing.jl` ecosystem.
The `Turing.jl` PPL is not fully stable and since developing our DSL, the `@submodel` macro has changed syntax and become a function with breaking changes in how it handles prefixes and in how it manages variables within a submodel, meaning our current implementation is not compatible with the latest `Turing.jl` release.
...
This allows for less overhead in model construction and easier to generalise components as distributions are more widely used in the Julia ecosystem when compared to submodels.
Other recent changes in `Turing.jl`, such as the primary workflow for observed data being to condition and fix  after model creation with no argument for observations in the original model, impact how data is conditioned on the model. 
This means not having to pass observations through layers of the model, potentially making it easier to build composable models.
...
Such capabilities would also facilitate formal approaches to modular Bayesian inference including Markov melding [@goudie2019markov] for combining submodels fitted separately, and cut likelihoods [@plummer2015cuts] for controlling feedback between modules.
This would enable the kind of methodological sharing that was difficult during the COVID-19 pandemic.
For instance, the observation model linking PCR prevalence to underlying incidence [@abbott2022estimating] could be packaged as a reusable submodel, allowing other analyses to incorporate these dynamics directly.
The handling of uncertainty and bias mitigation in policy-relevant analyses such as those assessing variant severity [@nyberg2022omicron] or informing intervention decisions [@davies21_associationcovidengland] could also be improved using these methods.
...
However, in practice, some of these utilities, such as profiling tools, can be difficult to use with `Turing.jl` when compared to similar tools offered in other ecosystems because the probabilistic programming language abstractions and macro expansions obscure the relationship between source code and runtime behaviour, making it challenging in our experience to identify performance bottlenecks in model components.
...
Methodological advances are also needed including for the joint estimation of interdependent epidemiological parameters, and the integration of individual and population-level observations.
The Julia [@Julia-2017] and `Turing.jl` [@fjelde2025turin] ecosystems need further development in areas such as debugging tools that can trace through model execution whilst preserving the semantic structure of probabilistic models, better error messages that relate numerical issues to model specification rather than low-level implementation details, standardised approaches for model validation and testing, and testing to make sure that new features are composable with composable components.
One sensible approach would be to develop a composable version of the Bayesian Regression modelling package [@brms], an R package for flexible Bayesian regression modelling, which could be used to develop composable components for Bayesian regression models.
This could then be used to validate other developments in the ecosystem, and provide a basis for developing composable components for other types of models, and provide useful tools for applied research.
...
Future surveillance programmes could be designed with composable modelling frameworks in mind, enabling estimates and model components to be shared and reused from initial data collection through to policy-relevant analyses, maximising the public health impact of large-scale data collection efforts.
Compositional frameworks also creates opportunities for large language model integration.
...
Composable infectious disease modelling offers a path to maintain statistical rigour whilst enabling rapid model adaptation and development.
By enabling domain experts to contribute specialised components without understanding entire modelling frameworks, such approaches could accelerate integration of diverse expertise during outbreak response.
...
Given the unpredictable nature of future infectious disease threats, investment in adaptable modelling infrastructure that can incorporate diverse data sources and domain knowledge is warranted.
