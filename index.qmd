...
Prior predictive samples (Figure @fig-figure3 A) show the behaviour of the piecewise constant by week process.
...
-   Standardised interfaces enable large language models to compose models programmatically whilst validation methods ensure statistical rigour.
-   High-level component interfaces reduce context requirements for language models, enabling deployment of smaller or less advanced models on local compute in resource-constrained settings.
-   Current struct manipulation xannot coordinate updates of related parameters such as model order and corresponding priors without manual intervention. Either have to avoid this pattern or solve.
 ...
 - Other recent changes in Turing.jl such as the primary workflow being conditioning an fixing with no argument for observations impact how data is conditioned on the model. This would mean not having to pass observations through layers of the model and so would make building composable models much easier.
 - In the current version there is limited support for conditioning on data i.e no helper functions whicyh in some settings like when latent delays are added can be an issue as the data that is simulated by the model will be truncated by the length of the maximum delay
 - `Turing.jl` has limited handling of numerical instability and so things like large counts being simulated can cause errors which makes evaluating and developing models frustrating
 - Struct interface adds overhead and ineffeciencies to the model. Not clear on the impact of this or how to improve. There are profiling tools in Julia but unclear how to use them wiuth Turing models.
 - We only partially implemented support for automated mappings between latent and observed states. Doing this well is tricky. 
 - Similarly we only added partial support for partial pooling approaches etc again hard. 
