...
This enables the statistical rigour of joint models, where all data is analysed together, whilst maintaining the flexibility to adapt analyses as new data sources become available.

We propose requirements for composable infectious disease modelling based on challenges from recent outbreaks, including communicating uncertainty, separation of processes so that they can be worked on by specialists in that domain, and the ability to combine models in stages.
...
Such systems could improve both routine surveillance and outbreak response: for endemic pathogens such as influenza, validated components could be refined across seasons for ongoing forecasting; during outbreaks, the same components could be reused, and if needed, rapidly adapted for novel pathogens.
...
As a consequence, modellers often have to re-implement model components in new models that are used for different situations even when underlying concepts are, in principle, the same.
...
This is also true in established collaborative routine forecasting efforts [@Reich2019-flusight; @Cramer2021-hub-dataset], where teams rarely share components despite common forecast targets.
...
These tools have enabled rigorous joint modelling and inference, reduced development effort and improved model quality compared with manual specification [@vandemeent2018introduction].
...
This allows flexibility but prevents full propagation of uncertainty and can introduce biased estimates compared with the rigour of fully joint models [@lison2024].
