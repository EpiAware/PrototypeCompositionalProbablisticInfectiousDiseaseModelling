...
We demonstrate our prototype compositional modelling DSL by recreating three published epidemiological analyses: "On the derivation of the renewal equation from an age-dependent branching process" [@mishra2020covid], "EpiNow2: Estimate Real-Time Case Counts and Time-Varying Epidemiological Parameters" [@abbott-epinow2-wellcomeopenres], and "Contemporary statistical inference for infectious disease models using Stan" [@chatzilena2019contemporary].

@fig-case-studies shows the model structure for the ARIMA example and each case study, illustrating how complex models are built by composing reusable components.
The ARIMA example panel shows how AR(2) and MA(1) components compose into ARIMA(2,1,1) as discussed in the last section.
Dashed arrows indicate component reuse across case studies: Mishra et al. reuses the AR(2) component for modelling the time-varying reproduction number, whilst the EpiNow2 replication reuses the full ARIMA structure with additional components for reporting delays and day-of-week effects.
Chatzilena et al. demonstrates an alternative infection process using an SIR ODE rather than a renewal model.
...
This enables the kind of methodological sharing that was difficult during the COVID-19 pandemic.
...
Combined with approaches such as Markov melding [@goudie2019markov] and cut likelihoods [@plummer2015cuts], this would enable improved handling of uncertainty and bias mitigation in policy-relevant analyses such as those assessing variant severity [@nyberg2022omicron] or informing intervention decisions [@davies21_associationcovidengland].
